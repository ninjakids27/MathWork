...
\section*{Core Backpropagation Formulas}

\begin{enumerate}
    \item \textbf{Error Function (Mean Squared Error - MSE)}:
    $$E = \frac{1}{2}\sum_{k} (t_k - y_k)^2$$

    \item \textbf{Output Layer Error Term ($\delta_k$)}:
    $$\delta_k = (t_k - y_k) \cdot f'(net_k)$$

    \item \textbf{Hidden Layer Error Term ($\delta_j$)}:
    $$\delta_j = f'(net_j) \cdot \sum_{k} (\delta_k w_{kj})$$
    
    \item \textbf{Weight Update Rule ($\Delta w_{ji}$)}:
    $$\Delta w_{ji} = \eta \cdot \delta_j \cdot y_i$$
    \centerline{New Weight: $w_{ji}^{new} = w_{ji}^{old} + \Delta w_{ji}$}

    \item \textbf{Bias Update Rule ($\Delta b_j$)}:
...
\subsection*{Variable Key}
\begin{itemize}
    \item $E$: Error (Loss)
    \item $t_k$: Target value for output unit $k$
    \item $y_k$: Predicted output value for unit $k$
    \item $f'(net)$: Derivative of the activation function with respect to the weighted input ($net$)
    \item $\delta$: Error Term
    \item $w_{kj}$: Weight connecting unit $j$ to unit $k$
