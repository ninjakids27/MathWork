...
\par{Backpropagation is the heart of ML and the most important and difficult
step for any ML project. The algorithm of backpropagation can vary from project to project
as optimizers are used differently for the notes I'm going to introduce it using 
gradient descent.}\\\\
\text{You first start with the loss function which we can use the MSE for}
$$\mathcal{L} = \frac{1}{n}\sum_{i=1}^n(Y_i-\hat{Y_i})^2$$
\par{Then we progress to something more abstract like calculating the loss for the output layer}
$$\delta_{\text{out}}=(t_k-y_k)\sigma'(\mathbf{N}_{n-1})$$
\par{The activation input is the last layer's input so it's like a forward prop, but the derivative
gets us the indicator of the gradient. Computing the hidden layers is a bit more complex but 
not a huge jump from the last formula.}
$$\delta_{\text{hidden}} = \sigma'(\mathbf{N})_{n-1}\sum_k(\delta_{out}\mathbf{W})$$
