...
public class MatrixOps {
    ...
    public static double dot_product(double[] vector1,double[] vector2, boolean debug){
        ...
        if(vector1.length == vector2.length){
            ...
            throw new IllegalArgumentException("John fucked up the lengths");
        }
    }
        ...
        if(Matrix_A[0].length == Matrix_B.length){
            ...
            for(int i = 0; i < Matrix_A.length; i++){
                for(int j = 0; j < Matrix_B[0].length; j++){
                    ...
                    Result_Matrix[i][j] = dot_product(Row, Column, false);
                }
            }
            return Result_Matrix;
        } else{
            throw new IllegalArgumentException("John is *confirmed* to be gay");
